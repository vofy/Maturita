...
- Fyzické části upočítače, umístěné přímo v počítačové skříni (nebo jsou součástí přenosného PC, nebo Allinone řešení)
...
### Patice (socket, slot) (socket je patice pro součástku, slot je konektor pro přímé připojení desky s plošným spojem)
- **Socket** - Místo, kam se připojuje procesor
	...
	- **Ball grid array (BGA)** - pracuje s kuličkami cínu, procesor je napájen na kontaktní plošky na základní desce	
...
- videovýstupy (viz grafická karta)
